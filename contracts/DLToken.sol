...
contract DLToken {
    ...
    // Helper functon to calculate the burn amount 5%
    function _burnPercentage(
        uint256 _amount
    ) internal pure returns (uint256 fivePercentage) {
    ...
    function transfer(address _reciever, uint256 _amountOfToken) external {
        ...
        uint256 fivePercentage = _burnPercentage(_amountOfToken);
